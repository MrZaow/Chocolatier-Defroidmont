<!DOCTYPE html>
<html lang="nl">
<head>
    <?php include("../../includes/head.php") ?>
    <title>Bestel onze producten - Chocolatier Defroidmont</title>
    <meta name="description" content="Bestel onze producten.">
</head>
<body>
    <?php include("../../includes/nl/header.php") ?>

    <section id="title" class="orange">
        <div class="container">
            <div class="row">
                <div class="col-sm-6">
                    <h1>Bestel onze producten</h1>
                </div>
            </div>
        </div>
    </section><!--/#title-->    

    <section class="container">
        <div class="row">
            <div class="col-md-6">
                <h2>Hoe u onze producten online kunt bestellen ?</h2>
                <p>
                    Om onze producten eenvoudig online te bestellen neemt u contact met ons op via het e- mailadres <a href="mailto:contact@chocolatier-defroidmont.be"> contact@chocolatier-defroidmont.be</a> met vermelding van de gewenste producten en hoeveelheden, evenals uw contactgegevens en het adres waar de levering plaats moet vinden. Wij zullen u een faktuur opsturen en na ontvangst van de betaling, zullen wij uw bestelling klaarmaken en verzenden.
                </p>
            </div>
            <div class="col-md-6">
                <img src="../../images/equipe-defroidmont.jpg" alt="Team van chocolaterie Defroidmont" class="img-responsive">
            </div>
        </div>
    </section>

    <?php include("../../includes/nl/bottom.php") ?>

    <?php include("../../includes/nl/footer.php") ?>

    <?php include("../../includes/script.php") ?>
</body>
</html>